...
export function StreamModal() {
  ...
  const { streams, addStream, updateStream, removeStream } = useStreams(activeQuarterId)

  const editing = editingStreamId ? streams.find(s => s._id === editingStreamId) ?? null : null
  ...
  async function save() {
    if (!name.trim()) return
    if (editing) {
      await updateStream({ streamId: editing._id, name: name.trim(), color })
    } else if (activeQuarterId) {
      await addStream({ name: name.trim(), color, quarterId: activeQuarterId })
    }
    close()
  }
  ...
  async function remove() {
    ...
    await removeStream({ streamId: editing._id })
    close()
  }
