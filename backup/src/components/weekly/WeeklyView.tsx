...
function initColumns(tasks: Task[]): Record<TaskStatus, string[]> {
  ...
  tasks.forEach(t => cols[t.status].push(t.id))
  return cols
}
...
export function WeeklyView({ week, onReturnToCurrent }: Props) {
  ...
  const reorderInWeek = useTaskStore(s => s.reorderInWeek)
  const openTask = useUIStore(s => s.openTask)

  const allTasks = useMemo(() => groups.flatMap(g => g.tasks), [groups])

  const taskById = useMemo(
    () => Object.fromEntries(allTasks.map(t => [t.id, t])) as Record<string, Task>,
    [allTasks]
  )
  const streamColorById = useMemo(() => {
    const map: Record<string, string> = {}
    groups.forEach(g => g.tasks.forEach(t => { map[t.id] = g.stream.color }))
    return map
  }, [groups])
  const streamNameById = useMemo(() => {
    const map: Record<string, string> = {}
    groups.forEach(g => g.tasks.forEach(t => { map[t.id] = g.stream.name }))
    return map
  }, [groups])
  ...
  function onDragEnd({ active }: DragEndEvent) {
    ...
    const allOrderedIds = STATUS_KEYS.flatMap(s => columns[s])
    reorderInWeek(quarter.id, week, allOrderedIds)
  }
          ...
          <div className="flex gap-5 px-6 py-5 mx-auto">
            {COLUMNS.map(col => (
              <KanbanColumn
                ...
                onOpenTask={id => {
                  const t = taskById[id]
                  if (t) openTask({ streamId: t.streamId, weekNumber: t.weekNumber }, t.id)
                }}
          ...
          {activeTask && (
            <div className="w-72 shadow-2xl rotate-1 opacity-95">
              <div className="flex overflow-hidden bg-white rounded-xl border border-indigo-200 shadow-lg">
                <CardContent
                  task={activeTask}
                  streamColor={streamColorById[activeTask.id] ?? '#94a3b8'}
                  streamName={streamNameById[activeTask.id] ?? ''}
                />
              </div>
            </div>
          )}
